...
const gettingType = (arrayofButtons) => {
    arrayofButtons.map((buttonSelected) => {
        buttonSelected.addEventListener("click", (event) => {
            ...
            const dataFiltered = window
                .allPokemon
                .dataFiltered(dataPoke, buttonType)
...
searcher.addEventListener('keyup', () => {
    ...
    let filteredByName = window
        .allPokemon
        .filterByName(dataPoke, nameAdded)
    printResult(filteredByName)
});

sortAtoZ.addEventListener('click', () => {
    let orderAtoZ = window
        .allPokemon
        .orderData(resultTotal);
    printResult(orderAtoZ);
})

sortZtoA.addEventListener('click', () => {
    let orderZtoA = window
        .allPokemon
        .orderDataZtoA(resultTotal);
